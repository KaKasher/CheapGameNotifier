...
class Hunter(commands.Cog):
    ...
    def get_game_record(self, url):
        if ALLKEYSHOPURL.lower() in url.lower():
            ...
            merchant_url = cheapest_row.find('div', {'class': 'offers-table-row-cell buy-btn-cell'}).find('a')['href']
            merchant_url = 'https:' + merchant_url

            return {'title': title, 'merchant': merchant, 'price': price, 'url': url, 'merchant_url': merchant_url}
            ...
            return {'title': title, 'merchant': 'steam', 'price': price, 'url': url, 'merchant_url': url}
    ...
    def db_add_game(self, game_record):
        try:
            with conn:
                c.execute("INSERT INTO games VALUES (:title, :merchant, :price, :url, :merchant_url)", game_record)
        except sqlite3.IntegrityError:
            pass
        except ValueError:
            pass
    ...
    def db_get_user_wish_comm(self, user_id, table=False):
        with conn:
            wish = c.execute("""SELECT title, wished_price, price, merchant, merchant_url
            FROM wishlist, games
    ...
    def db_update_game(self, game_record):
        with conn:
            ...
            SET title = :title, price = :price, merchant = :merchant, merchant_url = :merchant_url 
    ...
    def db_get_game_record(self, url=None, title=None):
        with conn:
            ...
            game_record = dict(zip(('title', 'merchant', 'price', 'url', 'merchant_url'), record))
            return game_record
    ...
    # Removes the wish from wishlist table for a given user provided a title, if no rows were affected it raises an error
    ...
    # Updates user's wished price for a given title in wishlist table, if no rows were affected it raises an error
    ...
    def db_get_merchant_url(self, url):
        with conn:
            c.execute("""SELECT merchant_url FROM games WHERE url = :url""", {'url': url})
            url = c.fetchone()
    ...
    async def wishdelete(self, ctx, *, title):
        ...
        await ctx.send(':boom: Wish deleted!')
    ...
    async def wishupdate(self, ctx, wished_price: float, *, title):
        ...
        url = self.db_get_game_record(title=title)['url']

        was_notified = 0
        self.db_set_notified(ctx.author.id, url)

        await ctx.send(':white_check_mark: Wish updated!')
    ...
    async def wishdelete_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            await ctx.send(':x: Title not found on your wishlist')
        else:
            await ctx.send(':x: Invalid syntax\n'
